...
This board is only designed to work with small DC-hobby motors and there is no documentation on exactly what motor driver chip is used.  The documentation only indicates that the maximum current is 1A continuous power and 1.5A for up to 5 seconds.  But it is not clear if this is per motor or total.  I suspect that if you glued a small heat fin to the unknown motor driver IC on the main board you could drive slightly larger motors.
