...
def update_display(counter, pot_value, delay):
    ...
    oled.fill_rect(0, 10, int((256 - pot_value)/2), 20, 1)
...
def splash_screen(counter, pot_value, delay):
    oled.fill(0)
    oled.blit(fb, 64, 0)
    oled.text('CoderDojo', 0, 0)
...
while current_pot == read_pot_u8():
    splash_screen(counter, current_pot, delay)
    counter += 1

# repeat forever
while True:
    pot_value = read_pot_u8() # read the value from the pot
