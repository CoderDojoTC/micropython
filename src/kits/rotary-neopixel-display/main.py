...
color_index = 0
color = red
colors =      (red,    orange,   yellow,   green,   blue,  cyan,   indigo,  caramel,   violet,   white)
color_names = ['red', 'orange', 'yellow', 'green', 'blue','cyan', 'indigo', 'caramel', 'violet', 'white']
...
while True:
    if mode == 0:
        ...
        for i in range (len(colors)):
            strip[(val+i) % (NUMBER_PIXELS)] = colors[i]
        
        # turn off the rest
        for i in range (len(colors),NUMBER_PIXELS):
            strip[(val+i) % (NUMBER_PIXELS)] = (0,0,0)
