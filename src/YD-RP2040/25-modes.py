...
# The YP-RP2040 board defines these pins
BUTTON_PIN = 24
BUILTIN_LED_PIN = 25

builtin_led = machine.Pin(BUILTIN_LED_PIN, Pin.OUT)
...
def rainbow_cycle(counter, wait):
    for i in range(NUMBER_PIXELS):
        color = i * int(255/NUMBER_PIXELS)
        strip[i] = wheel((color + counter*10) & 255)
    strip.write()
    sleep(wait)
...
while True:
    ...
    if mode != last_mode:
        ...
        last_mode = mode
    if mode == 0:
        rainbow_cycle(counter, base_delay)
    elif mode == 1:
        move_dot(counter, red, base_delay)
    elif mode == 2:
        move_dot(counter, green, base_delay)
    elif mode == 3:
        move_dot(counter, blue, base_delay)
    elif mode == 4:  
        comet_tail(counter, red, base_delay)
    elif mode == 5:  
        comet_tail(counter, green, base_delay)
    elif mode == 6:  
        comet_tail(counter, blue, base_delay)
    elif mode == 7:  
        candle(base_delay)
    elif mode == 8:  
        random_color(base_delay)
    elif mode == 9:  
        bounce(counter, red, base_delay)
    elif mode == 10: 
        running_lights(counter, blue, 3, base_delay*2)
    elif mode == 11: 
        rainbow_cycle(counter, base_delay)
    else:
